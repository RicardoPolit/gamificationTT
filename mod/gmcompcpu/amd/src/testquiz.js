 ...
 *
 * @module    mod_gmcompcpu/gmcompcpu
 * @class     gmcompcpu
...
define(['jquery','core/yui', 'core/notification', 'core/ajax'], function($, Y, notification, ajax) {
    ...
    $('#mod_gmcompcpu_fullscreen_button').on('click', function () {
        if (inFullscreen) {
            inFullscreen = false;
            smallscreen();
        } else {
    ...
    function playSound(soundName) {
        if (document.getElementById("mod_gmcompcpu_sound_on").checked) {
            var soundElement = document.getElementById("mod_gmcompcpu_sound_" + soundName);
    ...
    function fullscreen() {
        ...
        $("#mod_gmcompcpu_fullscreen_button").blur(); // The button pressed was still focused, so a blur is necessary.
    ...
    function showMenu() {
        ...
        if (questions !== null && questions.length > 0) {
            context.fillText(M.util.get_string('spacetostart', 'mod_gmcompcpu'), displayRect.width / 2, displayRect.height / 2);
            menuEvents();
        } else {
            context.fillText(M.util.get_string('emptyquiz', 'mod_gmcompcpu'), displayRect.width / 2, displayRect.height / 2);
        }
    }
    ...
    function endGame() {
        ajax.call([{
            methodname: 'mod_gmcompcpu_update_score',
            args: {gmcompcpuid: gmcompcpu, score: Math.trunc(score)},
    ...
    function startGame() {
        ...
        ajax.call([{
            methodname: 'mod_gmcompcpu_start_game',
            args: {gmcompcpuid: gmcompcpu},
            fail: notification.exception
        }]);
    ...
    function draw(context, displayRect, objects, particles, question) {
        ...
        if (player.alive) {
            ...
            context.fillText(M.util.get_string('score', 'mod_gmcompcpu',
                                               {
            ...
            context.font = "18px Audiowide";
            context.textAlign = 'center';
            context.fillText(M.util.get_string('endofgame', 'mod_gmcompcpu',
    ...
    function doInitialize(q, qid) {
        ...
        stage = document.getElementById("mod_gmcompcpu_game");
