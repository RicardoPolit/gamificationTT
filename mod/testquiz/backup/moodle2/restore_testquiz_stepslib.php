...
/**
 * Structure step to restore one testquiz activity.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_testquiz_activity_structure_step extends restore_activity_structure_step {

    /**
     * DB structure for a testquiz.
     */
    protected function define_structure() {
        ...
        $paths[] = new restore_path_element('testquiz', '/activity/testquiz');
        if ($userinfo) {
            $paths[] = new restore_path_element('testquiz_score', '/activity/testquiz/scores/score');
        }
    ...
    /**
     * Function to restore the testquiz activity
     * @param StdClass $data
     */
    protected function process_testquiz($data) {
        global $DB;
        ...
        if (count($category) > 1) {
            ...
            if (!empty($newcat)) {
                ...
                if (!$this->task->is_samesite() || $data->course != $oldcourse) {
                    ...
                    $this->log('question category ' . $category[0] . ' was associated with the testquiz ' .
        ...
        // Insert the testquiz record.
        $newitemid = $DB->insert_record('testquiz', $data);
        // Immediately after inserting "activity" record, call this.
        $this->apply_activity_instance($newitemid);
        $this->set_mapping('testquiz', $oldid, $newitemid);
    }
     ...
     * @param StdClass $data
     */
    protected function process_testquiz_score($data) {
        global $DB;

        $data = (object)$data;
        $oldid = $data->id;

        $data->testquizid = $this->get_new_parentid('testquiz');
        $data->userid = $this->get_mappingid('user', $data->userid);

        $newitemid = $DB->insert_record('testquiz_scores', $data);
        $this->set_mapping('testquiz_scores', $oldid, $newitemid, true); // Files by this itemname.
    }
    ...
    protected function after_execute() {
        // Add testquiz related files, no need to match by itemname (just internally handled context).
        $this->add_related_files('mod_testquiz', 'intro', null);
